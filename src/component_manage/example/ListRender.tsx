...
export default function ListRender() {
    ...
    const fiEmployess = employees.filter(item => item.department === '재무');

  return (
    <div>
        {listItems}
        {counts.map((item) => {
            return <h1 key={item}>반복작업</h1>
        })}
        {fiEmployess.map((item, index) => <h5 key={index}>{`이름 : ${item.name} 부서 : ${item.department}`}</h5>)}
    </div>
  )
}
